...
| 模式 | 適用情境 | 參數配置 | 前置需求 |
|------|----------|----------|----------|
| **Token 模式** | 雲端 CI（Azure Pipelines 託管代理程式） | 僅提供 **GitHub Token** | • GitHub Copilot 訂閱<br>• Fine-grained Personal Access Token 且具備 Copilot 唯讀權限<br>• **CI Token 模式**：在此 Task 之前執行 `npm install -g @github/copilot` （[安裝指南](https://docs.github.com/en/copilot/how-tos/copilot-cli/install-copilot-cli)） |
